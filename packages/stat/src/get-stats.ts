...
import type { GetStatsOptions, StatsResult } from './types';

export async function getStats(src: PathLike, dest: PathLike, opts?: GetStatsOptions): Promise<StatsResult> {
...
export function getStatsSync(src: PathLike, dest: PathLike, opts?: GetStatsOptions): StatsResult {
	...
	try {
		...
		if (err.code === 'ENOENT') return {srcStat, destStat: undefined};
		throw err;
	}
