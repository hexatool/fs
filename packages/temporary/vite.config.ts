...
export default defineConfig({
	build: {
		...
		lib: {
			...
			fileName: (format, entryName) =>
				`hexatool-fs-temporary-${entryName}.${format === 'es' ? 'mjs' : 'cjs'}`,
	...
	resolve: {
		alias: {
			...
			'@hexatool/fs-copy/async': resolve(__dirname, '../copy/src/index'),
			'@hexatool/fs-temporary/async': resolve(__dirname, './src/async'),
			...
			'@hexatool/fs-temporary': resolve(__dirname, './src/sync'),
